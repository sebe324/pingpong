...
void Game::getInput(Input* input){
    ...
    if(isDown(BUTTON_PAUSE)) {isDown(BUTTON_PAUSE)=false; paused=!paused; PlaySound(TEXT("C:\\sounds\\pause.wav"), NULL, SND_ASYNC);}
}
...
if(ball.didCollide(player[i])){
        PlaySound(TEXT("C:\\sounds\\pop.wav"), NULL, SND_ASYNC);
...
if(ball.getPosY()<=0 || ball.getPosY()+ball.getHeight()>=height){ball.setPosY(ball.getPosY()-10*Utils::isPositive(ball.getPosY()+ball.getHeight()-posY)); ball.setVelY(ball.getVelY()*-1); PlaySound(TEXT("C:\\sounds\\pop.wav"), NULL, SND_ASYNC);}
if(ball.getPosX()<0){
ball.setVelY(0);
ball.setVelX(450);
...
player[1].incScore();
if(player[1].getScore()>=player[0].getScore()+5)  PlaySound(TEXT("C:\\sounds\\hihihiha.wav"), NULL, SND_ASYNC);
else PlaySound(TEXT("C:\\sounds\\point.wav"), NULL, SND_ASYNC);
}
...
ball.setPosY((height-30)/2);
player[0].incScore();
if(player[0].getScore()>=player[1].getScore()+5)  PlaySound(TEXT("C:\\sounds\\hihihiha.wav"), NULL, SND_ASYNC);
else PlaySound(TEXT("C:\\sounds\\point.wav"), NULL, SND_ASYNC);
}
